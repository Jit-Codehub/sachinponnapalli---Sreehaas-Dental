...
class CustomPageContent(models.Model):
    ...
    featured_image = models.ImageField(upload_to="featured_images/%Y/%m/%d/", null=True, blank=True)
    status = models.CharField(max_length=20, choices=STATUS_CHOICE, default="draft")
...
class Doctor(models.Model):
    ...
    image = models.ImageField(upload_to="doctor_images/%Y/%m/%d/", null=True, blank=True)
    ...
    def __str__(self):
        if self.honorific_suffix not in self.name:
